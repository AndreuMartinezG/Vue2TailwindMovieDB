<template>
    <div class="flex flex-col w-2/3 justify-center items-center">
        <div class="flex w-full pt-4 justify-start">
            <a href="/tvshows" @click="backOption"
...
export default {
    ...
    created() {
        this.querryID = this.$ls.get('tvshowID')
    },
    ...
    methods: {
        backOption() {
            this.$ls.set('tvshowbackOption' , true)
        },
        ...
        movieDetailSearch() {
            ...
            fetch(API_URL + 'tv/' + this.querryID + '?language=en-US', options)
            ...
            fetch(API_URL + 'tv/' + this.querryID + '/videos?language=en-US', options)
                .then(response => response.json())
